<?php $__env->startSection('breadcrumb'); ?>
    <?php if(request()->routeIs('account.dashboard.index')): ?>
        <li><a href="<?php echo e(route('account.dashboard.index')); ?>"><?php echo e(clean(trans('cynoebook::account.links.my_account'))); ?></a></li>
        <li class="active"><?php echo e(clean(trans('cynoebook::account.links.my_ebook'))); ?></li>
        
    <?php else: ?>
        <li><a href="<?php echo e(route('account.dashboard.index')); ?>"><?php echo e(clean(trans('cynoebook::account.links.my_account'))); ?></a></li>
    <?php endif; ?>

    <?php echo $__env->yieldContent('account_breadcrumb'); ?>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('content'); ?>
    <?php if(setting('cynoebook_ad1_section_enabled')): ?>
       <?php echo $__env->make('public.home.sections.advertisement',['ad'=>setting('cynoebook_ad_1')], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php endif; ?> 
    
    <div class="row">
        <div class="my-account clearfix">
      
            <div class="col-md-3">
                <div class="sidebar-menu">
                    <ul class="list-inline">
                        <li class="<?php echo e(request()->routeIs('account.dashboard.index') ? 'active' : ''); ?>">
                            <a href="<?php echo e(route('account.dashboard.index')); ?>">
                                <i class="fa fa-book" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.my_ebook'))); ?>

                            </a>
                        </li>
                        <?php if(setting('enable_ebook_upload')): ?>
                        <li class="<?php echo e(request()->routeIs('ebooks.upload') ? 'active' : ''); ?>">
                            <a href="<?php echo e(route('ebooks.upload')); ?>">
                                <i class="fa fa-upload" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.upload_ebook'))); ?>

                            </a>
                        </li>
                        <li class="<?php echo e(( request()->routeIs('account.authors.index' ) || request()->routeIs('account.authors.create' ) || request()->routeIs('account.authors.edit' )  ) ? 'active' : ''); ?>">
                            <a href="<?php echo e(route('account.authors.index')); ?>">
                                <i class="fa fa-address-book" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.my_authors'))); ?>

                            </a>
                        </li>
                        <?php endif; ?>
                        
                        <li class="<?php echo e(request()->routeIs('account.favorite.index') ? 'active' : ''); ?>">
                            <a href="<?php echo e(route('account.favorite.index')); ?>">
                                <i class="fa fa-heart" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.my_favorite'))); ?>

                            </a>
                        </li>

                        <li class="<?php echo e(request()->routeIs('account.reviews.index') ? 'active' : ''); ?>">
                            <a href="<?php echo e(route('account.reviews.index')); ?>">
                                <i class="fa fa-comment" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.my_reviews'))); ?>

                            </a>
                        </li>

                        <li class="<?php echo e(request()->routeIs('user.profile.show') ? 'active' : ''); ?>">
                            <a target="_blank" href="<?php echo e(route('user.profile.show',auth()->user()->username)); ?>">
                                <i class="fa fa-user-circle-o" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.my_profile'))); ?>

                            </a>
                        </li>
                        
                        <li class="<?php echo e(request()->routeIs('account.profile.edit') ? 'active' : ''); ?>">
                            <a href="<?php echo e(route('account.profile.edit')); ?>">
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.edit_profile'))); ?>

                            </a>
                        </li>
                        
                        <li>
                            <a href="<?php echo e(route('logout')); ?>">
                                <i class="fa fa-sign-out" aria-hidden="true"></i>
                                <?php echo e(clean(trans('cynoebook::account.links.logout'))); ?>

                            </a>
                        </li>
                    </ul>
                </div>
            </div>

            <div class="col-md-9">
                <div class="clearfix"></div>

                <div class="content-right clearfix">
                    <?php echo $__env->yieldContent('content_right'); ?>
                </div>
            </div>
            
            
            
        </div>
    </div>
    <?php if(setting('cynoebook_ad2_section_enabled')): ?>
       <?php echo $__env->make('public.home.sections.advertisement',['ad'=>setting('cynoebook_ad_2')], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php endif; ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('public.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\Themes\Cynoebook\views/public/account/layout.blade.php ENDPATH**/ ?>