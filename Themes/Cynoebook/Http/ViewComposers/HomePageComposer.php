...
class HomePageComposer
{
    ...
    public function compose($view)
    {
        ...
        $view->with([
            ...
            'browsingHistoryEbooks' => $this->getBrowsingHistoryEbook(),
    ...
    private function getBrowsingHistoryEbook()
    {
    ...
    private function getBrowsingEbook(){

        $cookieValue= Cookie::get('EbookViewed');
        $cookieValue=json_decode($cookieValue,true); 
        $ids_ordered=implode(',',$cookieValue);
        
        return Ebook::with(['user','categories','authors'])->whereIn('id',$cookieValue)->orderByRaw("FIELD(id, $ids_ordered)")
        ->take(setting('cynoebook_inspired_by_your_browsing_history_section_total_ebooks', 12))
        ->get();
